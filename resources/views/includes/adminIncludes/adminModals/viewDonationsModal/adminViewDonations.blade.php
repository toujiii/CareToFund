    ...
    <div
        ...
        <div class=" mt-4 h-100 overflow-y-auto border-1 rounded-md border-gray-400 p-2 ">
            <table class="w-full ">
                ...
                <tbody>
                    @foreach($charity->donators->sortByDesc('created_at') as $donator)
                        <tr class="border-b border-white hover:bg-light">
                            <td class="p-2 text-sm">{{ ucfirst(explode(' ', $donator->user->name)[0]) }}</td>
                            <td class="p-2 text-sm">{{ $donator->created_at->format('F d, Y') }}</td>
                            <td class="p-2 text-sm text-green-300">PHP {{ number_format($donator->amount, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
